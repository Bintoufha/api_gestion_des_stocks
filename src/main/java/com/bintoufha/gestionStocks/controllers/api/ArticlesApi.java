...
public interface ArticlesApi {
    ...
    @Operation(summary = "Enregistrer un article", description = "Cette méthode permet d'enregistrer ou de modifier un article dans la base de données")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Créé ou modifié un article")
    })
    ArticleSaveDto save(@RequestBody ArticleSaveDto dto);

    @GetMapping(value = APP_ROOT + "/articles/recherche/{uuidArticle}", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Rechercher un article par UUID", description = "Cette méthode permet de rechercher un article par son identifiant UUID")
    @ApiResponses(value = {
            @ApiResponse(responseCode = "200", description = "Article trouvé"),
            @ApiResponse(responseCode = "404", description = "Aucun article trouvé avec l'identifiant fourni")
    })
    ArticleListDto findByUUID(@PathVariable("uuidArticle") UUID uuid);

    // @GetMapping(value = APP_ROOT + "/article/code_article/{codeArticle}",
    ...
    // @ApiResponses(value = {
    // @ApiResponse(responseCode = "200", description = "Article trouvé"),
    // @ApiResponse(responseCode = "404", description = "Aucun article trouvé avec
    // le code fourni")
    // })
    // ArticlesDto findByCodeArticle(@PathVariable("codeArticle") String
    // codeArticle);

    @GetMapping(value = APP_ROOT + "/articles/allArticle", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Lister tous les articles", description = "Cette méthode permet de lister tous les articles enregistrés")
    @ApiResponses(value = {
    ...
    @GetMapping(value = APP_ROOT
            + "/articles/historique/vente/{uuidArticle}", produces = MediaType.APPLICATION_JSON_VALUE)
    List<LigneVenteDto> findHistoriqueVente(@PathVariable("uuidArticle") UUID uuidArticle);

    @GetMapping(value = APP_ROOT
            + "/articles/historique/commandeClient/{uuidArticle}", produces = MediaType.APPLICATION_JSON_VALUE)
    List<LigneCommandeClientSaveDto> findHistoriqueCommandeCLients(@PathVariable("uuidArticle") UUID uuidArticle);

    @GetMapping(value = APP_ROOT
            + "/articles/historique/commandeFournisseur/{uuidArticle}", produces = MediaType.APPLICATION_JSON_VALUE)
    List<LigneCommandeFournisseurSaveDto> findHistoriqueCommandeFournisseurs(
            @PathVariable("uuidArticle") UUID uuidArticle);

//    @GetMapping(value = APP_ROOT
//            + "/articles/filter/categorie/{uuidCategorie}", produces = MediaType.APPLICATION_JSON_VALUE)
//    List<ArticlesDto> findAllArticleByCategorieUuid(@PathVariable("uuidCategorie") UUID uuidCategorie);

    @DeleteMapping(value = APP_ROOT
            + "/articles/supprimerArticle/{uuidArticles}", produces = MediaType.APPLICATION_JSON_VALUE)
    @Operation(summary = "Supprimer un article", description = "Cette méthode permet de supprimer un article par UUID")
    @ApiResponses(value = {
